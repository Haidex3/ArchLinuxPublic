...
function get-scheme-json -a scheme_dir
    ...
    set -l theme (string trim (cat $scheme_dir/current-theme.txt))
    set pairs $pairs "\"theme\":\"$theme\""

    # colors
    for line in (cat $scheme_dir/colors.txt)
...
inotifywait -q -e close_write,moved_to,create -m $scheme_dir | while read dir events file
    test "$dir$file" = $scheme_dir/colors.txt; and ~/scripts/firefox/message.sh (get-scheme-json $scheme_dir)
end
